...
void printf6(char* s, char* a1, char* a2, char* a3, char* a4, char* a5,
             char* a6);
void printf7(char* s, char* a1, char* a2, char* a3, char* a4, char* a5,
             char* a6, char* a7);
...
uint64_t WORDSIZE =
    8; // (double) word size in bytes, must be the same as SIZEOFUINT64
...
void init_library() {
    ...
    // determine actual size of uint64_t*
    SIZEOFUINT64STAR =
        (uint64_t)((uint64_t**)SELFIE_URL + 1) - (uint64_t)SELFIE_URL;
...
void create_symbol_table_entry(uint64_t which, char* string, uint64_t line,
                               uint64_t class, uint64_t type, uint64_t value,
                               uint64_t address);
...
void reset_symbol_tables() {
    global_symbol_table =
        (uint64_t*)zmalloc(HASH_TABLE_SIZE * SIZEOFUINT64STAR);
...
uint64_t e_phnum = 1; // number of program header entries (code and data
                      // segment; TODO: extend to 2)
...
uint64_t NUMBEROFLEAFPTES =
    512; // number of leaf page table entries == PAGESIZE / SIZEOFUINT64STAR

uint64_t PAGETABLETREE = 1; // two-level page table is default

// ------------------------ GLOBAL VARIABLES -----------------------

uint64_t total_page_frame_memory =
    0; // total amount of memory available for frames
...
void init_interpreter() {
    ...
    *(EXCEPTIONS + EXCEPTION_SEGMENTATIONFAULT) =
        (uint64_t) "segmentation fault";
...
void reset_source_profile() {
    ...
    stores_per_instruction =
        zmalloc(code_size / INSTRUCTIONSIZE * SIZEOFUINT64);
}
...
uint64_t* get_used_list_head(uint64_t* context) {
    return (uint64_t*)*(context + 21);
}
uint64_t* get_free_list_head(uint64_t* context) {
    return (uint64_t*)*(context + 22);
}
...
void set_used_list_head(uint64_t* context, uint64_t* used_list_head) {
    *(context + 21) = (uint64_t)used_list_head;
}
void set_free_list_head(uint64_t* context, uint64_t* free_list_head) {
    *(context + 22) = (uint64_t)free_list_head;
}
...
void init_system() {
    ...
    if (SIZEOFUINT64INBITS != 64) {
        if (SIZEOFUINT64INBITS == 32) {
            ...
            e_phentsize =
                32; // size of program header entry 32 bytes (ELFCLASS32)
        } else
    ...
    if (is_boot_level_zero()) {
        ...
        // caution: the name of the executable must not have an extension to
        // make this work try opening executable with zeroed flags which likely
        // fails but just on Windows
        if (signed_less_than(
                sign_extend(open(selfie_name, 0, 0), SYSCALL_BITWIDTH), 0))
            WINDOWS = 1;
    }
}
    ...
    if (output_buffer) {
        ...
        *character_buffer = c;
        ...
        if (write(output_fd, character_buffer, 1) == 1) {
            ...
            // write failed
            if (output_fd != 1) {
                // failed write was not to the console which has file descriptor
                // 1 to report the error we may thus still write to the console
                output_fd = 1;
        ...
        while (load_character(s, i) != 0) {
            if (load_character(s, i) != '%') {
                ...
                if (p < 10) {
                    ...
                    if (p > 0) {
                        // using integer_buffer here is ok since we are not
                        // using print_integer
                        itoa((uint64_t)a % ten_to_the_power_of(p),
                             integer_buffer, 10, 0, 0);
...
void printf1(char* s, char* a1) { print_format0(s, print_format1(s, 0, a1)); }
...
void printf3(char* s, char* a1, char* a2, char* a3) {
    print_format0(
        s, print_format1(s, print_format1(s, print_format1(s, 0, a1), a2), a3));
}
...
void printf4(char* s, char* a1, char* a2, char* a3, char* a4) {
    print_format0(
        ...
        print_format1(
            s,
            print_format1(s, print_format1(s, print_format1(s, 0, a1), a2), a3),
            a4));
}
    ...
    print_format0(
        s, print_format1(
               ...
               print_format1(
                   s,
                   print_format1(
                       s, print_format1(s, print_format1(s, 0, a1), a2), a3),
                   a4),
               a5));
}
               ...
               print_format1(
                   ...
                   print_format1(
                       ...
                       print_format1(
                           s,
                           print_format1(
                               s, print_format1(s, print_format1(s, 0, a1), a2),
                               a3),
                           a4),
                       a5),
                   a6),
...
void print_line_number(char* message, uint64_t line) {
    printf4("%s: %s in %s in line %u: ", selfie_name, message, source_name,
            (char*)line);
}
    ...
    while (1) {
        if (in_single_line_comment) {
            ...
            while (character == CHAR_ASTERISK) {
                // look for "*/" by looping over consecutive '*' counting them
                // as ignored characters
                number_of_ignored_characters = number_of_ignored_characters + 1;
            ...
            if (in_multi_line_comment) {
                // keep track of line numbers for error reporting and code
                // annotation
                if (character == CHAR_LF)
                    // only line feeds count towards line numbers, not carriage
                    // returns
            ...
            // count the characters in comments as ignored characters including
            // '/' in "*/"
            number_of_ignored_characters = number_of_ignored_characters + 1;
            ...
            // character found that is not whitespace and not occurring in a
            // comment
            return character;
    }
}
    ...
    if (find_next_character() != CHAR_EOF) {
        if (symbol != SYM_DIVISION) {
            ...
            if (is_character_letter()) {
                ...
                // hack... take any string of letters+digits as the same literal
                // so that the checking in atoi triggers... otherwise we would
                // need to do the checks here?
                while (is_character_letter() +
                       is_character_digit(BASE_HEXADECIMAL)) {
...
void create_symbol_table_entry(uint64_t which_table, char* string,
                               uint64_t line, uint64_t class, uint64_t type,
...
uint64_t is_undefined_procedure(uint64_t* entry) {
    ...
    if (get_class(entry) == PROCEDURE) {
        // library procedures override declared or defined procedures
        library_entry = search_symbol_table(library_symbol_table,
                                            get_string(entry), PROCEDURE);
...
uint64_t load_variable_or_big_int(char* variable_or_big_int, uint64_t class) {
    ...
    if (is_signed_integer(offset, 12)) {
        ...
        emit_load(current_temporary(), current_temporary(),
                  sign_extend(get_bits(offset, 0, 12), 12));
    }
...
void load_integer(uint64_t value) {
    ...
    if (is_signed_integer(value, 32)) {
        ...
        // integers with value < -2^31 or value >= 2^31 are stored in data
        // segment
        ...
        if (entry == (uint64_t*)0) {
            ...
            create_symbol_table_entry(GLOBAL_TABLE, integer, line_number,
...
void load_string(char* string) {
    ...
    create_symbol_table_entry(GLOBAL_TABLE, string, line_number, STRING,
...
uint64_t procedure_call(uint64_t* entry, char* procedure) {
    ...
    if (entry == (uint64_t*)0) {
        ...
        create_symbol_table_entry(GLOBAL_TABLE, procedure, line_number,
                                  PROCEDURE, type, 0, code_size);
...
uint64_t compile_term() {
    ...
    while (is_mult_or_div_or_rem()) {
        ...
        if (operator_symbol == SYM_ASTERISK)
            emit_mul(previous_temporary(), previous_temporary(),
                     current_temporary());
...
uint64_t compile_simple_expression() {
    ...
    while (is_plus_or_minus()) {
        ...
        if (operator_symbol == SYM_PLUS) {
            ...
            emit_add(previous_temporary(), previous_temporary(),
                     current_temporary());
            ...
            if (ltype == UINT64STAR_T) {
                if (rtype == UINT64_T) {
                    ...
                    // UINT64STAR_T - UINT64STAR_T
                    // pointer arithmetic: (left_term - right_term) /
                    // SIZEOFUINT64
                    emit_sub(previous_temporary(), previous_temporary(),
                             current_temporary());
...
void compile_statement() {
    ...
    // variable "=" expression | call
        ...
        if (symbol == SYM_LPARENTHESIS) {
            ...
            entry =
                get_variable_or_big_int(variable_or_procedure_name, VARIABLE);
...
void compile_variable(uint64_t offset) {
    ...
    if (symbol == SYM_IDENTIFIER) {
        ...
        create_symbol_table_entry(LOCAL_TABLE, identifier, line_number,
        ...
        create_symbol_table_entry(LOCAL_TABLE, "missing variable name",
        ...
        if (entry == (uint64_t*)0)
            ...
            if (is_undefined) {
                ...
                if (string_compare(procedure, "main")) {
                    // first source containing main procedure provides binary
                    // name
        ...
        if (symbol == SYM_VOID) {
            ...
            if (symbol == SYM_IDENTIFIER) {
                ...
                if (symbol == SYM_LPARENTHESIS)
                    ...
                    entry = search_global_symbol_table(
                        variable_or_procedure_name, VARIABLE);

                    if (entry == (uint64_t*)0) {
                        data_size = data_size + WORDSIZE;

                        create_symbol_table_entry(
                            GLOBAL_TABLE, variable_or_procedure_name,
                            current_line_number, VARIABLE, type, initial_value,
                            -data_size);
                    } else {
...
void emit_bootstrapping() {
    ...
    */
    ...
    if (report_undefined_procedures()) {
        ...
        // avoid sign extension that would result in an additional sub
        // instruction
...
void selfie_compile() {
    ...
    // copy "main" string into zeroed word to obtain unique hash
    create_symbol_table_entry(GLOBAL_TABLE, string_copy("main"), 0, PROCEDURE,
                              UINT64_T, 0, 0);
    ...
    while (link) {
        if (number_of_remaining_arguments() == 0)
            ...
            // assert: source_name is mapped and not longer than
            // MAX_FILENAME_LENGTH

            source_fd =
                sign_extend(open(source_name, O_RDONLY, 0), SYSCALL_BITWIDTH);
            ...
            printf4("%s: %u characters read in %u lines and %u comments\n",
                    selfie_name, (char*)number_of_read_characters,
                    (char*)line_number, (char*)number_of_comments);

            printf4("%s: with %u(%.2u%%) characters in %u actual symbols\n",
                    selfie_name,
                    (char*)(number_of_read_characters -
                            number_of_ignored_characters),
                    (char*)percentage_format(number_of_read_characters,
                                             number_of_read_characters -
                                                 number_of_ignored_characters),
                    (char*)number_of_scanned_symbols);

            printf4(
                "%s: %u global variables, %u procedures, %u string literals\n",
                selfie_name, (char*)number_of_global_variables,
                (char*)number_of_procedures, (char*)number_of_strings);

            printf6(
                "%s: %u calls, %u assignments, %u while, %u if, %u return\n",
                selfie_name, (char*)number_of_calls,
                (char*)number_of_assignments, (char*)number_of_while,
                (char*)number_of_if, (char*)number_of_return);
        }
    }
    ...
    printf4(
        "%s: %u bytes generated with %u instructions and %u bytes of data\n",
        selfie_name, (char*)code_size + data_size,
...
void print_register_name(uint64_t reg) { print(get_register_name(reg)); }
...
void print_instruction_counter(uint64_t counter, uint64_t ins) {
    printf3(
        "%s: %u(%.2u%%)", get_mnemonic(ins), (char*)counter,
...
uint64_t load_code(uint64_t caddr) { return *(code_binary + caddr / WORDSIZE); }
...
uint64_t load_data(uint64_t daddr) { return *(data_binary + daddr / WORDSIZE); }
...
void fixup_relative_BFormat(uint64_t from_address) {
    ...
    store_instruction(from_address, encode_b_format(code_size - from_address,
                                                    get_rs2(instruction),
                                                    get_rs1(instruction),
...
void fixlink_relative(uint64_t from_address, uint64_t to_address) {
    ...
    while (from_address != 0) {
        previous_address =
            get_immediate_j_format(load_instruction(from_address));
...
void emit_data_word(uint64_t data, uint64_t offset,
    ...
    if (data_line_number != (uint64_t*)0)
        *(data_line_number + (data_size + offset) / WORDSIZE) =
            source_line_number;
}
...
void emit_string_data(uint64_t* entry) {
    ...
    while (i < l) {
        // CAUTION: at boot levels higher than 0, s is only accessible
        // in C* at word granularity, not individual characters
        emit_data_word(*((uint64_t*)s), get_address(entry) + i,
...
uint64_t* encode_elf_header() {
    ...
    // store all data necessary for creating a minimal and valid file and
    // program header
...
uint64_t open_write_only(char* name) {
    ...
    if (WINDOWS)
        // use Windows flags
        fd = sign_extend(open(name, WINDOWS_O_BINARY_CREAT_TRUNC_WRONLY,
        ...
        if (signed_less_than(fd, 0))
            ...
            fd = sign_extend(open(name, LINUX_O_CREAT_TRUNC_WRONLY,
                                  S_IRUSR_IWUSR_IRGRP_IROTH),
                             SYSCALL_BITWIDTH);
    }
    ...
    if (number_of_read_bytes == ELF_HEADER_SIZE) {
        if (validate_elf_header(ELF_header)) {
            number_of_read_bytes =
                sign_extend(read(fd, code_binary, code_size), SYSCALL_BITWIDTH);

            if (number_of_read_bytes == code_size) {
                number_of_read_bytes = sign_extend(
                    read(fd, data_binary, data_size), SYSCALL_BITWIDTH);
...
void emit_exit() {
    create_symbol_table_entry(LIBRARY_TABLE, "exit", 0, PROCEDURE, VOID_T, 0,
                              code_size);
...
void emit_read() {
    create_symbol_table_entry(LIBRARY_TABLE, "read", 0, PROCEDURE, UINT64_T, 0,
                              code_size);
        ...
        if (is_virtual_address_valid(vbuffer, WORDSIZE))
            if (is_data_stack_heap_address(context, vbuffer))
                if (is_virtual_address_mapped(get_pt(context), vbuffer)) {
                    ...
                    actually_read = sign_extend(read(fd, buffer, bytes_to_read),
                                                SYSCALL_BITWIDTH);
...
void emit_write() {
    create_symbol_table_entry(LIBRARY_TABLE, "write", 0, PROCEDURE, UINT64_T, 0,
                              code_size);
        ...
        if (is_virtual_address_valid(vbuffer, WORDSIZE))
            if (is_data_stack_heap_address(context, vbuffer))
                if (is_virtual_address_mapped(get_pt(context), vbuffer)) {
                    ...
                    actually_written = sign_extend(
                        write(fd, buffer, bytes_to_write), SYSCALL_BITWIDTH);
...
void emit_open() {
    create_symbol_table_entry(LIBRARY_TABLE, "open", 0, PROCEDURE, UINT64_T, 0,
                              code_size);
...
void implement_openat(uint64_t* context) {
    ...
    if (down_load_string(context, vfilename, filename_buffer)) {
        if (flags == MAC_O_CREAT_TRUNC_WRONLY)
            ...
            fd = sign_extend(open(filename_buffer, flags, mode),
                             SYSCALL_BITWIDTH);
...
void emit_malloc() {
    ...
    create_symbol_table_entry(LIBRARY_TABLE, "malloc", 0, PROCEDURE,
                              UINT64STAR_T, 0, code_size);

    // on boot levels higher than 0, zalloc falls back to malloc
    // assuming that page frames are zeroed on boot level zero
    create_symbol_table_entry(LIBRARY_TABLE, "zalloc", 0, PROCEDURE,
                              UINT64STAR_T, 0, code_size);
    ...
    // copy "_bump" string into zeroed word to obtain unique hash
    create_symbol_table_entry(GLOBAL_TABLE, string_copy("_bump"), 1, VARIABLE,
...
void emit_switch() {
    create_symbol_table_entry(LIBRARY_TABLE, "hypster_switch", 0, PROCEDURE,
                              UINT64STAR_T, 0, code_size);
...
uint64_t* tlb(uint64_t* table, uint64_t vaddr) {
    ...
    if (debug_tlb)
        printf5(
            "%s: tlb access:\n vaddr: %p\n page:  %p\n frame: %p\n paddr: %p\n",
            selfie_name, (char*)vaddr, (char*)(page * PAGESIZE), (char*)frame,
            (char*)paddr);
...
void emit_fetch_stack_pointer() {
    create_symbol_table_entry(LIBRARY_TABLE, "fetch_stack_pointer", 0,
...
void emit_fetch_global_pointer() {
    create_symbol_table_entry(LIBRARY_TABLE, "fetch_global_pointer", 0,
                              PROCEDURE, UINT64_T, 0, code_size);
...
void emit_fetch_data_segment_size_interface() {
    create_symbol_table_entry(LIBRARY_TABLE, "fetch_data_segment_size", 0,
                              PROCEDURE, UINT64_T, 0, code_size);
    ...
    if (program_break > get_program_break(context)) {
        ...
        *(get_regs(context) + REG_A0) =
        ...
        store_virtual_memory(get_pt(context),
                             get_data_seg_end_gc(context) - SIZEOFUINT64,
...
uint64_t* allocate_new_memory(uint64_t* context, uint64_t size) {
    ...
    if (is_gc_library(context)) {
        ...
        // assert: smalloc_system is a bump pointer allocator that may reuse
        // memory
...
uint64_t* allocate_memory_selfie(uint64_t* context, uint64_t size) {
    ...
    // stack is not zeroed! using two successive gc_malloc calls (library
    // variant) leads to having the same variables as with the previous call and
    // therefore we might have a reachable pointer which is not actually
    // reachable. to fix this, we set these variables to 0:
    object = (uint64_t*)0;
    metadata = (uint64_t*)0;
...
void mark(uint64_t* context) {
    ...
    // assert: temporary registers do not contain any reference to gc_heap
    // memory selfie saves all relevant temporary registers on stack, see
    // procedure_prologue().
...
void print_gc_profile(uint64_t* context) {
    ...
    printf5(
        ...
        selfie_name, (char*)ratio_format(gc_mem_mallocated, MEGABYTE),
        (char*)gc_num_mallocated, (char*)gc_num_gced_mallocs,
        (char*)gc_num_reused_mallocs);
    printf4("%s: gc:      %.2uMB(%.2u%%) reused in %u reused mallocs\n",
            selfie_name, (char*)ratio_format(gc_mem_reused, MEGABYTE),
    ...
    printf6("%s: gc:      %.2uMB(%.2u%%) allocated in %u mallocs (%u gced, %u ungced)\n",
            ...
            (char*)gc_num_gced_mallocs, (char*)gc_num_ungced_mallocs);
    printf4("%s: gc:      %.2uMB(%.2u%%) allocated in %u gced mallocs\n",
            selfie_name, (char*)ratio_format(gc_mem_objects, MEGABYTE),
            (char*)percentage_format(gc_mem_mallocated, gc_mem_objects),
            (char*)gc_num_gced_mallocs);
    printf4("%s: gc:      %.2uMB(%.2u%%) allocated in %u ungced mallocs",
            selfie_name, (char*)ratio_format(gc_mem_metadata, MEGABYTE),
...
void print_code_line_number_for_instruction(uint64_t address, uint64_t offset) {
    if (code_line_number != (uint64_t*)0)
        printf1("(~%u)", (char*)*(code_line_number +
...
void print_lui() {
    ...
    printf3("%s %s,%x", get_mnemonic(is), get_register_name(rd),
            (char*)sign_shrink(imm, 20));
}
...
void print_addi() {
    ...
    printf4("%s %s,%s,%d", get_mnemonic(is), get_register_name(rd),
...
void print_load() {
    print_code_context_for_instruction(pc);
    printf4("%s %s,%d(%s)", get_mnemonic(is), get_register_name(rd), (char*)imm,
    ...
    if (is_virtual_address_valid(vaddr, WORDSIZE))
        if (is_virtual_address_mapped(pt, vaddr)) {
            if (is_system_register(rd))
                printf2(",mem[%x]=%x |- ", (char*)vaddr,
                        (char*)load_virtual_memory(pt, vaddr));
            else
                printf2(",mem[%x]=%d |- ", (char*)vaddr,
                        (char*)load_virtual_memory(pt, vaddr));
            print_register_value(rd);
...
void print_store() {
    print_code_context_for_instruction(pc);
    printf4("%s %s,%d(%s)", get_mnemonic(is), get_register_name(rs2),
            (char*)imm, get_register_name(rs1));
}
    ...
    if (is_virtual_address_valid(vaddr, WORDSIZE)) {
        if (is_valid_segment_write(vaddr)) {
            if (is_virtual_address_mapped(pt, vaddr)) {
                ...
                // and individually
                *(stores_per_instruction + a) =
                    *(stores_per_instruction + a) + 1;
            } else
                throw_exception(EXCEPTION_PAGEFAULT,
                                get_page_of_virtual_address(vaddr));
        } else
...
void print_jalr() {
    print_code_context_for_instruction(pc);
    printf4("%s %s,%d(%s)", get_mnemonic(is), get_register_name(rd),
            (char*)signed_division(imm, INSTRUCTIONSIZE),
            get_register_name(rs1));
}
...
void print_data_line_number() {
    if (data_line_number != (uint64_t*)0)
        printf1("(~%u)",
                (char*)*(data_line_number + (pc - code_size) / SIZEOFUINT64));
}
...
void print_register_value(uint64_t reg) {
    if (is_system_register(reg))
        ...
        printf3("%s=%d(%x)", get_register_name(reg), (char*)*(registers + reg),
                (char*)*(registers + reg));
}
...
void throw_exception(uint64_t exception, uint64_t fault) {
    if (get_exception(current_context) != EXCEPTION_NOEXCEPTION)
        if (get_exception(current_context) != exception) {
            printf2("%s: context %p throws exception: ", selfie_name,
    ...
    if (is_virtual_address_valid(pc, INSTRUCTIONSIZE)) {
        if (is_code_address(current_context, pc)) {
            ...
            if (pc % WORDSIZE == 0)
                ...
                ir = get_high_instruction(
                    load_virtual_memory(pt, pc - INSTRUCTIONSIZE));
...
uint64_t print_per_instruction_counter(uint64_t total, uint64_t* counters,
    ...
    if (a != UINT64_MAX) {
        ...
        *(counters + a / INSTRUCTIONSIZE) = 0;
        ...
        printf3(",%u(%.2u%%)@%x", (char*)c, (char*)percentage_format(total, c),
                (char*)a);
...
void print_access_profile(char* message, char* padding, uint64_t reads,
    ...
    if (reads + writes > 0) {
        ...
        printf7("%s: %s%s%u,%u,%u[%.2u]\n", selfie_name, message, padding,
                (char*)(reads + writes), (char*)reads, (char*)writes,
                (char*)ratio_format(reads, writes));
    }
}
...
void print_register_memory_profile() {
    ...
    while (reg < NUMBEROFREGISTERS) {
        if (is_stack_register(reg)) {
            stack_register_reads =
                stack_register_reads + *(reads_per_register + reg);
...
void print_profile(uint64_t* context) {
    ...
    printf3("%s: summary: %u executed instructions [%.2u%% nops]\n",
            ...
            (char*)percentage_format(get_total_number_of_instructions(),
                                     get_total_number_of_nops()));
    printf3("%s:          %.2uMB allocated in %u mallocs\n", selfie_name,
            (char*)ratio_format(mc_brk, MEGABYTE), (char*)sc_brk);
    printf4(
        "%s:          %.2uMB(%.2u%% of %.2uMB) actually accessed\n",
        selfie_name, (char*)ratio_format(mc_mapped_heap, MEGABYTE),
    ...
    if (get_total_number_of_instructions() > 0) {
        ...
        print_per_instruction_profile("loops:   ", iterations,
                                      iterations_per_loop);
        print_per_instruction_profile("loads:   ", ic_load,
...
void init_context(uint64_t* context, uint64_t* parent, uint64_t* vctxt) {
    ...
    if (PAGETABLETREE == 0)
        ...
        set_pt(context,
               zmalloc(NUMBEROFPAGES / NUMBEROFLEAFPTES * SIZEOFUINT64STAR));

    // reset page table cache
    set_lowest_lo_page(context, 0);
    set_highest_lo_page(context, get_lowest_lo_page(context));
    set_lowest_hi_page(context, get_page_of_virtual_address(
                                    VIRTUALMEMORYSIZE * GIGABYTE - WORDSIZE));
    set_highest_hi_page(context, get_lowest_hi_page(context));

    if (parent != MY_CONTEXT) {
        set_code_seg_start(context, load_virtual_memory(get_pt(parent),
                                                        code_seg_start(vctxt)));
        set_code_seg_size(
            context, load_virtual_memory(get_pt(parent), code_seg_size(vctxt)));
        set_data_seg_start(context, load_virtual_memory(get_pt(parent),
                                                        data_seg_start(vctxt)));
        set_data_seg_size(
            context, load_virtual_memory(get_pt(parent), data_seg_size(vctxt)));
        set_heap_seg_start(context, load_virtual_memory(get_pt(parent),
                                                        heap_seg_start(vctxt)));
...
void save_context(uint64_t* context) {
    ...
    if (get_parent(context) != MY_CONTEXT) {
        ...
        store_virtual_memory(parent_table, program_counter(vctxt),
                             get_pc(context));
        ...
        while (r < NUMBEROFREGISTERS) {
            store_virtual_memory(parent_table, (uint64_t)(vregs + r),
                                 *(pregs + r));
        ...
        // garbage collector state (only necessary if context is gced by
        // different gcs)
...
void map_page(uint64_t* context, uint64_t page, uint64_t frame) {
    ...
    if (frame != 0) {
        ...
        if (get_frame_for_page(table, page) == 0) {
            ...
            if (page <= get_page_of_virtual_address(get_program_break(context) -
                                                    WORDSIZE)) {
                set_lowest_lo_page(
                    context, lowest_page(page, get_lowest_lo_page(context)));
                set_highest_lo_page(
                    context, highest_page(page, get_highest_lo_page(context)));
            } else {
                set_lowest_hi_page(
                    context, lowest_page(page, get_lowest_hi_page(context)));
                set_highest_hi_page(
                    context, highest_page(page, get_highest_hi_page(context)));
            }
...
void restore_region(uint64_t* context, uint64_t* table, uint64_t* parent_table,
    ...
    while (lo < hi) {
        if (is_virtual_address_mapped(
            ...
            map_page(context, lo,
                     get_frame_for_page(parent_table,
                                        get_page_of_virtual_address(frame)));
        }
...
void restore_context(uint64_t* context) {
    ...
    if (get_parent(context) != MY_CONTEXT) {
        // download changes in virtual context in parent address space to
        // context
        parent_table = get_pt(get_parent(context));
        ...
        set_pc(context,
               load_virtual_memory(parent_table, program_counter(vctxt)));
        ...
        while (r < NUMBEROFREGISTERS) {
            *(pregs + r) =
                load_virtual_memory(parent_table, (uint64_t)(vregs + r));

            r = r + 1;
        }

        set_program_break(
            context, load_virtual_memory(parent_table, program_break(vctxt)));

        set_exception(context,
                      load_virtual_memory(parent_table, exception(vctxt)));
        set_fault(context, load_virtual_memory(parent_table, fault(vctxt)));

        set_exit_code(context,
                      load_virtual_memory(parent_table, exit_code(vctxt)));

        table = (uint64_t*)load_virtual_memory(parent_table, page_table(vctxt));

        // assert: virtual context page table is only mapped from beginning up
        // and end down
        ...
        set_gcs_in_period(
            context, load_virtual_memory(parent_table, gcs_in_period(vctxt)));
        set_use_gc_kernel(
            context, load_virtual_memory(parent_table, use_gc_kernel(vctxt)));
    }
...
uint64_t pexcess() {
    if (pavailable())
        return 1;
             ...
             2 * total_page_frame_memory)
        // tolerate twice as much memory mapped on demand than physically
        // available
        return 1;
    else
        return 0;
...
void map_and_store(uint64_t* context, uint64_t vaddr, uint64_t data) {
    ...
    if (is_virtual_address_mapped(get_pt(context), vaddr) == 0)
        map_page(context, get_page_of_virtual_address(vaddr),
                 (uint64_t)palloc());
    ...
    while (1) {
        ...
        if (get_parent(from_context) != MY_CONTEXT) {
            ...
            if (get_exception(from_context) == EXCEPTION_PAGEFAULT) {
                printf2("%s: context %s threw uncaught exception: ",
                        selfie_name, get_name(from_context));
                print_exception(get_exception(from_context),
                                get_fault(from_context));
                println();
...
uint64_t selfie_run(uint64_t machine) {
    ...
    printf3("%s: selfie executing %s with %uMB physical memory", selfie_name,
            binary_name, (char*)(total_page_frame_memory / MEGABYTE));
    ...
    if (GC_ON) {
        ...
        if (GC_REUSE)
            print("reusing memory");
        else
            print("not reusing memory");
    }
...
uint64_t selfie(uint64_t extras) {
    if (number_of_remaining_arguments() == 0)
        ...
        printf1("%s: This is Jan Jecmen's Selfie\n", selfie_name);
        printf2("%s: this is the selfie system from %s with\n", selfie_name,
                SELFIE_URL);
        printf3(
            "%s: %u-bit unsigned integers and %u-bit pointers on boot level ",
            selfie_name, (char*)SIZEOFUINT64INBITS,
